...
const main = async () => {
  ...
  const toEnable = campaigns.filter(c => c.status === 3);

  console.log(`Found ${campaigns.length} campaigns. Need to enable ${toEnable.length}.`);
  ...
  const batchSize = 50;
  let enabled = 0;
  for (let i = 0; i < toEnable.length; i += batchSize) {
    const batch = toEnable.slice(i, i + batchSize);
    ...
    console.log(`Enabled ${enabled}/${toEnable.length}...`);
  }

  console.log('Done. All PAUSED campaigns are now ENABLED.');
};
