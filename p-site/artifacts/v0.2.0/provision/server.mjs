...
// uuid -> { pty, buf, lastQrDataUrl, lastQrAt, lastLoginAt, status, lastStatusAt, lastStatusRaw, qrSeq }

function ensureSession(uuid){
  let s = sessions.get(uuid);
  if (!s) {
    s = { pty:null, buf:'', lastQrDataUrl:null, lastQrAt:null, lastLoginAt:0, status:false, lastStatusAt:null, lastStatusRaw:null, qrSeq:0 };
    sessions.set(uuid, s);
  }
  return s;
}
...
function startLogin(uuid, { force=false } = {}){
  ...
  term.onData((d) => {
    s.buf += d;
    // keep buffer bounded
    if (s.buf.length > 200000) s.buf = s.buf.slice(-120000);

    // extract QR blocks
    const blocks = extractQrBlocks(s.buf);
    if (blocks.length) {
      const last = blocks[blocks.length-1];
      // hash to avoid regenerating same png
      const h = crypto.createHash('sha1').update(last.join('\n')).digest('hex');
      if (s._lastQrHash !== h) {
        s._lastQrHash = h;
        try {
          s.lastQrDataUrl = asciiQrToPngDataUrl(last, { scale: 7, border: 3 });
          s.lastQrAt = nowIso();
          s.qrSeq += 1;
        } catch {
          // ignore
