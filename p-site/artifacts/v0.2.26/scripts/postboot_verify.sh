...
ensure_agent_baseline(){
  ...
  # Ensure sessions dir is creatable (prevents EACCES on inbound handling).
  install -d -m 700 -o ubuntu -g ubuntu /home/ubuntu/.openclaw/agents/main/sessions 2>/dev/null || true
...
PY
  ...
  # NOTE: dmPolicy=open requires allowFrom to include "*" (OpenClaw validation).
  sudo -u ubuntu env HOME=/home/ubuntu OPENCLAW_STATE_DIR=/home/ubuntu/.openclaw /home/ubuntu/.npm-global/bin/openclaw config set channels.whatsapp.allowFrom '["*"]' >/dev/null 2>&1 || true
  sudo -u ubuntu env HOME=/home/ubuntu OPENCLAW_STATE_DIR=/home/ubuntu/.openclaw /home/ubuntu/.npm-global/bin/openclaw config set channels.whatsapp.dmPolicy open >/dev/null 2>&1 || true
...
# HARD gate: pool onboarding must accept DMs.
# dmPolicy=open requires allowFrom includes "*".
DM_OK=$(python3 - <<'PY'
import json
...
dm=str(wa.get('dmPolicy') or '')
allow=wa.get('allowFrom')
if dm!='open':
  print('0'); raise SystemExit(0)
if not isinstance(allow,list) or '*' not in [str(x) for x in allow]:
  print('0'); raise SystemExit(0)
print('1')
PY
)
if [[ "$DM_OK" != "1" ]]; then
  ok=false; errs+=("whatsapp DM gate not satisfied: expected dmPolicy=open + allowFrom includes *");
fi
...
export CHK_DM_GATE=$([ "$DM_OK" = "1" ] && echo 1 || echo 0)
...
# Presence of the memory store file (owner spec: must exist after bootstrap).
export CHK_MEMORY_SQLITE_OK=$([ -f /home/ubuntu/.openclaw/memory/memory.sqlite ] && echo 1 || echo 0)
...
j={
  ...
  'memory_store_sqlite_present': os.environ.get('CHK_MEMORY_SQLITE_OK')=='1',
