...
if 'BOTHook: inboundguard v2.13 (whatsapp)' not in s2:
  ...
  pos = s2.find(anchor)
  if pos > 0:
    v21 = "\n\t\t\t// BOTHook: inboundguard v2.13 (whatsapp)\n\t\t\ttry {\n\t\t\t\tif (!group) {\n\t\t\t\t\tconst rawBody = String(body || '').trim();\n\t\t\t\t\tconst isSelf = Boolean(selfE164 && from && String(from).replace(/\D+/g,'') === String(selfE164).replace(/\D+/g,''));\n\t\t\t\t\tif (!isSelf && Boolean(msg.key?.fromMe)) {\n\t\t\t\t\t\t// skip\n\t\t\t\t\t} else {\n\t\t\t\t\t\tlet uuid = null;\n\t\t\t\t\t\tlet pLink = null;\n\t\t\t\t\t\ttry {\n\t\t\t\t\t\t\tconst fs = await import('node:fs');\n\t\t\t\t\t\t\tconst t = fs.readFileSync('/opt/bothook/UUID.txt','utf8');\n\t\t\t\t\t\t\tconst m = t.match(/uuid=([a-zA-Z0-9-]{8,80})/);\n\t\t\t\t\t\t\tuuid = m ? m[1] : null;\n\t\t\t\t\t\t\tconst lm = t.match(/https?:\\/\\/\\S+/);\n\t\t\t\t\t\t\tpLink = lm ? lm[0] : null;\n\t\t\t\t\t\t} catch {}\n\t\t\t\t\t\tconst cpBase = (process.env.BOTHOOK_API_BASE || 'https://p.bothook.me').replace(/\\/$/, '');\n\t\t\t\t\t\tlet st = {};\n\t\t\t\t\t\ttry {\n\t\t\t\t\t\t\tconst fs = await import('node:fs');\n\t\t\t\t\t\t\tst = JSON.parse(fs.readFileSync('/opt/bothook/state.json','utf8'));\n\t\t\t\t\t\t} catch { st = {}; }\n\t\t\t\t\t\tst.autoreply = st.autoreply || {};\n\t\t\t\t\t\tst.autoreply.externalReplied = st.autoreply.externalReplied || {};\n\t\t\t\t\t\tconst saveState = async () => {\n\t\t\t\t\t\t\ttry {\n\t\t\t\t\t\t\t\tconst fs = await import('node:fs');\n\t\t\t\t\t\t\t\tfs.mkdirSync('/opt/bothook', { recursive: true });\n\t\t\t\t\t\t\t\tfs.writeFileSync('/opt/bothook/state.json', JSON.stringify(st, null, 2) + '\\n');\n\t\t\t\t\t\t\t} catch {}\n\t\t\t\t\t\t};\n\t\t\t\t\t\tconst sendText = async (jid, text) => {\n\t\t\t\t\t\t\tawait sock.sendMessage(jid, { text });\n\t\t\t\t\t\t};\n\t\t\t\t\t\t// Delivered gate: once /opt/bothook/DELIVERED.json exists, exit guide mode.\n\t\t\t\t\t\tlet delivered = false;\n\t\t\t\t\t\ttry { const fs2 = await import('node:fs'); delivered = fs2.existsSync('/opt/bothook/DELIVERED.json'); } catch {}\n\t\t\t\t\t\t// Delivered mode: external contacts must be silent; do not fall through to model.\n\t\t\t\t\t\tif (delivered && !isSelf && senderE164) { try { console.log('[bothook-inboundguard] delivered: ignore external'); } catch {} ; continue; }\n\t\t\t\t\t\tif (!delivered && isSelf && rawBody && !rawBody.startsWith('sk-')) {\n\t\t\t\t\t\t\t\t\t\t\t\t\t\tlet lang = 'en';\n\t\t\t\t\t\t\ttry { const mm = String(pLink || '').match(/[?&]lang=([^&#]+)/i); if (mm) lang = decodeURIComponent(mm[1] || 'en').toLowerCase(); } catch {}\n\t\t\t\t\t\t\tlet msg = `[bothook] Next: paste your OpenAI API key here as ONE line starting with sk- (self-chat only).\\nLink: ${pLink || (uuid ? `https://p.bothook.me/p/${uuid}?lang=${lang}` : '')}`;\n\t\t\t\t\t\t\ttry {\n\t\t\t\t\t\t\t\tconst rr = await fetch(`${cpBase}/api/i18n/whatsapp-prompts?lang=${encodeURIComponent(lang)}`, { redirect: 'follow' });\n\t\t\t\t\t\t\t\tconst tt = await rr.text();\n\t\t\t\t\t\t\t\tconst jj = JSON.parse(tt);\n\t\t\t\t\t\t\t\tconst pp = jj && jj.ok ? (jj.prompts || {}) : {};\n\t\t\t\t\t\t\t\tconst vars = { uuid: uuid || '', p_link: pLink || (uuid ? `https://p.bothook.me/p/${uuid}?lang=${lang}` : '') };\n\t\t\t\t\t\t\t\tif (pp.guide_key_paid) {\n\t\t\t\t\t\t\t\t\tmsg = String(pp.guide_key_paid);\n\t\t\t\t\t\t\t\t\tfor (const k of Object.keys(vars)) msg = msg.split(`{{${k}}}`).join(String(vars[k]));\n\t\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\t} catch {}\n\t\t\t\t\t\t\ttry { console.log(`[bothook-inboundguard] self msg lang=${lang} len=${String(msg||'').length}`); } catch {}\n								await sendText(remoteJid, msg);\n\t\t\t\t\t\t\tcontinue;\n\t\t\t\t\t\t}\n\t\t\t\t\t\tif (isSelf && rawBody.startsWith('sk-') && uuid) {\n\t\t\t\t\t\t\tconst key = rawBody.split(/\\s+/)[0];\n\t\t\t\t\t\t\tlet msg2 = '[bothook] Verifying OpenAI key…';\n\t\t\t\t\t\t\ttry {\n\t\t\t\t\t\t\t\tconst r = await fetch(`${cpBase}/api/key/verify`, { method: 'POST', headers: { 'content-type':'application/json' }, body: JSON.stringify({ uuid, provider: 'openai', key }) });\n\t\t\t\t\t\t\t\tconst j = await r.json().catch(() => ({}));\n\t\t\t\t\t\t\t\tmsg2 = j.message || (j.verified ? '[bothook] OpenAI Key verified ✅' : `[bothook] OpenAI Key verify failed: ${j.detail || j.error || 'unknown'}`);\n\t\t\t\t\t\t\t} catch {\n\t\t\t\t\t\t\t\tmsg2 = '[bothook] OpenAI Key verify failed: network/server error';\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t\tawait sendText(remoteJid, msg2);\n\t\t\t\t\t\t\tcontinue;\n\t\t\t\t\t\t}\n\t\t\t\t\t\tif (!delivered && !isSelf && senderE164) {\n\t\t\t\t\t\t\tconst k = String(senderE164);\n												if (st.autoreply.externalReplied[k]) { try { console.log('[bothook-inboundguard] ext already replied'); } catch {} ; continue; }\n\t\t\t\t\t\t\tif (!st.autoreply.externalReplied[k]) {\n\t\t\t\t\t\t\t\tst.autoreply.externalReplied[k] = new Date().toISOString();\n\t\t\t\t\t\t\t\tawait saveState();\n\t\t\t\t\t\t\t\tlet lang2 = 'en';\n									try { const mm2 = String(pLink || '').match(/[?&]lang=([^&#]+)/i); if (mm2) lang2 = decodeURIComponent(mm2[1] || 'en').toLowerCase(); } catch {}\n									let promo = `[bothook] The owner is activating a private WhatsApp AI assistant (dedicated server).\n\nLearn more: https://bothook.me`;\n									try {\n										const st2 = await (async()=>{ try { const r = await fetch(`${cpBase}/api/delivery/status?uuid=${encodeURIComponent(uuid||'')}`); const t = await r.text(); return JSON.parse(t);} catch { return null; } })();\n										const rr2 = await fetch(`${cpBase}/api/i18n/whatsapp-prompts?lang=${encodeURIComponent(lang2)}`, { redirect: 'follow' });\n										const tt2 = await rr2.text();\n										const jj2 = JSON.parse(tt2);\n										const pp2 = jj2 && jj2.ok ? (jj2.prompts || {}) : {};\n										const vars2 = { region: String((st2&&st2.region)||''), cpu: String((st2&&st2.cpu)||''), ram_gb: String((st2&&st2.ram_gb)||''), disk_gb: String((st2&&st2.disk_gb)||'') };\n										if (pp2.promo_external) {\n											promo = String(pp2.promo_external);\n											for (const k of Object.keys(vars2)) promo = promo.split(`{{${k}}}`).join(String(vars2[k]));\n										}\n									} catch {}\n									try { console.log(`[bothook-inboundguard] ext promo lang2=${lang2} len=${String(promo||'').length}`); } catch {}\n																		let sent = false;\n									try { await sendText(remoteJid, promo); sent = true; } catch (e) { try { console.log('[bothook-inboundguard] ext send err '+String(e)); } catch {} }\n									if (sent) { st.autoreply.externalReplied[k] = new Date().toISOString(); await saveState(); }\n\n\t\t\t\t\t\t\t\tcontinue;\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t}\n\t\t\t\t\t}\n\t\t\t\t}\n\t\t\t} catch {}\n"
