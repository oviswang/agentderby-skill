...
# IMPORTANT: be conservative to avoid self-inflicted restart storms during QR/login.
PROVISION_HEALTHZ_TIMEOUT_SEC=${WATCHDOG_PROVISION_HEALTHZ_TIMEOUT_SEC:-6}
PROVISION_FAIL_N=${WATCHDOG_PROVISION_FAIL_N:-3}
PROVISION_FAIL_STATE_FILE="/run/bothook_provision_healthz_fail.state"

fails=0
if [[ -f "$PROVISION_FAIL_STATE_FILE" ]]; then
  fails=$(cat "$PROVISION_FAIL_STATE_FILE" 2>/dev/null || echo 0)
fi

if curl -fsS --max-time "$PROVISION_HEALTHZ_TIMEOUT_SEC" http://127.0.0.1:18999/healthz >/dev/null 2>&1; then
  # recover/reset
  if (( fails != 0 )); then
    log "recover provision_healthz_ok: fails=${fails}->0"
  fi
  echo 0 > "$PROVISION_FAIL_STATE_FILE" 2>/dev/null || true
else
  fails=$((fails+1))
  echo "$fails" > "$PROVISION_FAIL_STATE_FILE" 2>/dev/null || true
  log "WARN provision_healthz_fail: fails=${fails}/${PROVISION_FAIL_N}"
  if (( fails >= PROVISION_FAIL_N )); then
