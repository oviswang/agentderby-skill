...
main(){
  ...
  # Hard guard: prevent provision server from ever starting again after delivery.
  # bothook-provision.service has ConditionPathExists=!/opt/bothook/LOGIN_AUTHORITY.control-plane
  # so creating this file permanently blocks future start attempts.
  touch /opt/bothook/LOGIN_AUTHORITY.control-plane 2>/dev/null || true
  chmod 644 /opt/bothook/LOGIN_AUTHORITY.control-plane 2>/dev/null || true
