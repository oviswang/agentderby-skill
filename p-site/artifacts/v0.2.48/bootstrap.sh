...
ARTIFACT_BASE_URL="${ARTIFACT_BASE_URL:-https://p.bothook.me/artifacts/latest}"
...
ensure_openclaw(){
  ...
  local pinned_ver="${OPENCLAW_PINNED_VERSION:-2026.3.7}"
...
main(){
  ...
  # Local patch scripts for OpenClaw (verified)
  mkdir -p "$INSTALL_DIR/scripts"
  fetch_verified "scripts/patch_openclaw_suppress_missing_key_warning.sh" "$INSTALL_DIR/scripts/patch_openclaw_suppress_missing_key_warning.sh" "$INSTALL_DIR/artifacts/sha256sums.txt"
  fetch_verified "scripts/patch_openclaw_enable_memory_search_openai.sh" "$INSTALL_DIR/scripts/patch_openclaw_enable_memory_search_openai.sh" "$INSTALL_DIR/artifacts/sha256sums.txt"
  chmod +x "$INSTALL_DIR/scripts/patch_openclaw_suppress_missing_key_warning.sh" "$INSTALL_DIR/scripts/patch_openclaw_enable_memory_search_openai.sh" || true
  ...
  mkdir -p /home/ubuntu/.openclaw/workspace/hooks/bothook-memory-init
  ...
  fetch_verified "hooks/bothook-memory-init/handler.ts" /home/ubuntu/.openclaw/workspace/hooks/bothook-memory-init/handler.ts "$INSTALL_DIR/artifacts/sha256sums.txt"
  ...
  # Fetch post-link fixer (verified)
  fetch_verified "scripts/wa_postlink_fixer.sh" "$INSTALL_DIR/bin/wa_postlink_fixer.sh" "$INSTALL_DIR/artifacts/sha256sums.txt"
  chmod +x "$INSTALL_DIR/bin/wa_postlink_fixer.sh"
  ...
  fetch_verified "systemd/bothook-wa-postlink-fixer.service" "$INSTALL_DIR/artifacts/bothook-wa-postlink-fixer.service" "$INSTALL_DIR/artifacts/sha256sums.txt"
  fetch_verified "systemd/bothook-wa-postlink-fixer.timer" "$INSTALL_DIR/artifacts/bothook-wa-postlink-fixer.timer" "$INSTALL_DIR/artifacts/sha256sums.txt"
  fetch_verified "systemd/bothook-wa-postlink-fixer.path" "$INSTALL_DIR/artifacts/bothook-wa-postlink-fixer.path" "$INSTALL_DIR/artifacts/sha256sums.txt"
  ...
  install -m 0644 "$INSTALL_DIR/artifacts/bothook-wa-postlink-fixer.service" "$SYSTEMD_DIR/bothook-wa-postlink-fixer.service"
  install -m 0644 "$INSTALL_DIR/artifacts/bothook-wa-postlink-fixer.timer" "$SYSTEMD_DIR/bothook-wa-postlink-fixer.timer"
  install -m 0644 "$INSTALL_DIR/artifacts/bothook-wa-postlink-fixer.path" "$SYSTEMD_DIR/bothook-wa-postlink-fixer.path"
...
# HARD-SET WhatsApp DM gate for onboarding reliability.
# Base images may already contain openclaw.json with dmPolicy=pairing; do not rely on "file missing" templates.
ch=j.get('channels') if isinstance(j.get('channels'), dict) else {}
wa=ch.get('whatsapp') if isinstance(ch.get('whatsapp'), dict) else {}
wa['enabled']=True
wa['dmPolicy']='open'
wa['allowFrom']=['*']
wa.setdefault('groupPolicy','allowlist')
ch['whatsapp']=wa
j['channels']=ch
...
PY
  ...
  sudo -u ubuntu /home/ubuntu/.npm-global/bin/openclaw config set update.auto.enabled false >/dev/null 2>&1 || true
...
PY
  ...
  sudo -u ubuntu /home/ubuntu/.npm-global/bin/openclaw config set hooks.internal.entries.bothook-memory-init.enabled true >/dev/null 2>&1 || true
  ...
  # Pre-create the memorySearch store so readiness can be verified and first-use doesn't race permissions.
  # IMPORTANT: use explicit absolute paths (do not rely on ~ / HOME) to avoid sudo env drift.
  install -d -m 700 -o ubuntu -g ubuntu /home/ubuntu/.openclaw/memory 2>/dev/null || true
  if [[ ! -f /home/ubuntu/.openclaw/memory/memory.sqlite ]]; then
    sudo -u ubuntu bash -lc ': > /home/ubuntu/.openclaw/memory/memory.sqlite' || true
  fi
  chown ubuntu:ubuntu /home/ubuntu/.openclaw/memory/memory.sqlite 2>/dev/null || true
  chmod 600 /home/ubuntu/.openclaw/memory/memory.sqlite 2>/dev/null || true

  # Enable embedding-based semantic memorySearch (OpenAI embeddings).
  # NOTE: does NOT change gateway/channels/models; safe if OPENAI_API_KEY not present yet.
  bash "$INSTALL_DIR/scripts/patch_openclaw_enable_memory_search_openai.sh" || true
  ...
  # node-pty is a native module; ensure build toolchain exists.
  apt-get update -y
  apt-get install -y build-essential python3 make g++

  chown -R ubuntu:ubuntu "$INSTALL_DIR/provision"
  sudo -u ubuntu bash -lc "cd '$INSTALL_DIR/provision' && npm install --omit=dev"
  # Hard check: ensure node-pty native module loads (prevents crash loops that break inbound).
  sudo -u ubuntu bash -lc "cd '$INSTALL_DIR/provision' && node -e \"import('node-pty').then(()=>console.log('pty_ok')).catch(e=>{console.error(e);process.exit(1)})\""
  ...
  # Ensure BOTHook evidence dir is writable by ubuntu (plugins write cheap readiness markers here).
  ...
  # Integrity gate input: write expected sha256 manifest for critical installed files.
  # postboot_verify.sh will refuse READY if this file is missing or mismatched.
  {
    echo "# BOTHook integrity manifest (generated by bootstrap)"
    echo "# generated_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    for p in \
      /opt/bothook/bin/openclaw_watchdog.sh \
      /opt/bothook/bin/openclaw-gateway-start.sh \
      /opt/bothook/ops-scripts/fix_openclaw_credentials_perms.sh \
      /etc/systemd/system/openclaw-gateway.service \
      /etc/systemd/system/bothook-provision.service \
      /etc/systemd/system/bothook-openclaw-watchdog.service \
      /etc/systemd/system/bothook-openclaw-watchdog.timer \
      ; do
      if [[ -f "$p" ]]; then
  ...
  } >/opt/bothook/SHA256SUMS.expected
  chmod 644 /opt/bothook/SHA256SUMS.expected 2>/dev/null || true
  ...
  if [[ ! -f "$INSTALL_DIR/evidence/postboot_verify.done" ]]; then
    ...
    # Reboot for P0.2 acceptance, but DELAY it slightly so the control-plane can run
    # post-bootstrap validations over SSH before the connection drops.
    log "Triggering one-time reboot for P0.2 acceptance (delayed 25s)"
