...
export default {
  ...
  activate(api: OpenClawPluginApi) {
    ...
    // Short welcome: send ONCE per linked self account (me.e164) to reduce "post-link silence".
    // Design goals:
    // - minimal dependencies (no uuid/pay link/specs)
    // - best-effort retry until sent (user trust)
    // - dedupe per me.e164 (option A)
    const SHORT_WELCOME = [
    ...
    (async () => {
      ...
      const tick = async () => {
        try {
          ...
          const selfE164 = await getSelfE164(api);
          if (!selfE164) return true;
          const key = sha256(selfE164).slice(0, 12);
          const marker = `${EVID_DIR}/short_welcome_sent_${key}`;
          if (fs.existsSync(marker)) return false;

          // Use self number as JID target (plugin runtime will route correctly for whatsapp).
          await sendWhatsApp(api, selfE164, SHORT_WELCOME);
          fs.mkdirSync(EVID_DIR, { recursive: true });
      ...
      // Loop (async) until success or timeout.
      const loop = async () => {
        while (true) {
