...
const AUTH_PROFILES_PATH = '/home/ubuntu/.openclaw/agents/main/agent/auth-profiles.json';
...
function writeOpenAiKeyToAuthProfiles(openAiKey: string) {
  // Keep format consistent with postboot_verify.sh baseline.
  // Never log the key.
  try {
    const dir = '/home/ubuntu/.openclaw/agents/main/agent';
    fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
  } catch {}

  try {
    let j: any = null;
    try { j = JSON.parse(safeRead(AUTH_PROFILES_PATH) || 'null'); } catch { j = null; }

    if (!j || typeof j !== 'object') j = { version: 1, profiles: {}, order: {} };
    j.version = j.version || 1;
    j.profiles = j.profiles && typeof j.profiles === 'object' ? j.profiles : {};
    j.order = j.order && typeof j.order === 'object' ? j.order : {};

    // Ensure profile slot exists.
    const id = 'openai:manual';
    const p = j.profiles[id] && typeof j.profiles[id] === 'object' ? j.profiles[id] : { type: 'api_key', provider: 'openai', key: '' };
    p.type = 'api_key';
    p.provider = 'openai';
    p.key = String(openAiKey || '');
    j.profiles[id] = p;

    // Ensure order references it.
    if (!Array.isArray(j.order.openai)) j.order.openai = [];
    if (!j.order.openai.includes(id)) j.order.openai = [id, ...j.order.openai].filter((v: any, i: number, a: any[]) => a.indexOf(v) === i);

    fs.writeFileSync(AUTH_PROFILES_PATH, JSON.stringify(j, null, 2) + '\n', { mode: 0o600 });
    try { fs.chmodSync(AUTH_PROFILES_PATH, 0o600); } catch {}
    writeEvidence('auth_profiles_write.json', { ts: nowIso(), ok: true, path: AUTH_PROFILES_PATH });
  } catch (e) {
    writeEvidence('auth_profiles_write.json', { ts: nowIso(), ok: false, error: String(e?.message || e) });
  }
}
...
async function reportInstanceEvent(controlPlane: string, uuid: string, event_type: string, payload: any) {
  try {
    const body = { uuid, event_type, payload: payload || {} };
    const r = await postJson(`${controlPlane}/api/instance/event`, body, 12000);
    writeEvidence('instance_event_last.json', { ts: nowIso(), uuid, event_type, ok: Boolean(r?.ok), status: r?.status ?? null });
    return r;
  } catch (e) {
    writeEvidence('instance_event_last.json', { ts: nowIso(), uuid, event_type, ok: false, error: String(e?.message || e) });
  ...
  activate(api: OpenClawPluginApi) {
    ...
    async function proactivePushTick() {
      ...
      if (paid) {
        ...
        if (verified && !st.autoreply.key_verified_success_sent_at) {
          ...
          await reportInstanceEvent(controlPlane, uuid, 'KEY_VERIFIED_SUCCESS_SENT', { lang, kind: 'key_verified_success' });
          return;
        }
        ...
        if (!verified && !st.autoreply.proactive_paid_guide_sent_at) {
          ...
          await reportInstanceEvent(controlPlane, uuid, 'GUIDE_KEY_PAID_SENT', { lang, kind: 'guide_key_paid' });
          return;
        }
      }
      ...
      if (!paid && status === 'BOUND_UNPAID' && !st.autoreply.welcome_full_sent_at) {
        ...
        if (wr.ok && text) {
          ...
          await reportInstanceEvent(controlPlane, uuid, 'WELCOME_UNPAID_SENT', { lang, kind: 'welcome_unpaid' });
          return;
        }
      }
    }
    ...
    api.on('message_received', async (event, ctx) => {
      try {
        ...
        if (paid) {
          // IMPORTANT: WhatsApp may insert newlines/whitespace into long pasted keys.
          // Always normalize by removing all whitespace before validation + verification.
          const normalized = content.replace(/\s+/g, '');
          if (!looksLikeOpenAiKey(normalized)) {
            // Not a key (after normalization) → fall through to guide repeat.
          } else {
          ...
          // Debug evidence (no key material): helps diagnose false negatives.
          writeEvidence('key_verify_last.json', {
            ts: nowIso(),
            uuid,
            ok: Boolean(vr?.ok),
            status: vr?.status ?? null,
            verified: Boolean(vr?.json?.verified),
            error: vr?.json?.error ?? null,
            detail: vr?.json?.detail ?? null
          });
          if (vr?.json?.verified === true) {
            ...
            st.autoreply.key_verified_success_sent_at = nowIso();
            st.autoreply.cutover_after_key_scheduled_at = nowIso();
            saveState(st);

            // Persist the OpenAI key locally so the agent can actually reply after cutover.
            writeOpenAiKeyToAuthProfiles(key);

            // After key verified, run delivered cutover locally (idempotent) then report to control-plane.
            // Do this asynchronously to avoid delaying the WhatsApp reply.
            setTimeout(() => {
              try {
                if (fs.existsSync('/opt/bothook/DELIVERED') || fs.existsSync('/opt/bothook/DELIVERED.json')) {
                  writeEvidence('cutover_after_key_skip.json', { ts: nowIso(), uuid, reason: 'already_delivered_marker' });
                  return;
                }

                const controller = String(self || '').trim();
                const args = ['-E', 'env', `BOTHOOK_UUID=${uuid}`, `BOTHOOK_CONTROLLER_E164=${controller}`, '/opt/bothook/bin/cutover_delivered.sh'];
                const child = spawn('sudo', args, { stdio: 'ignore' });

                child.on('exit', async (code) => {
                  try {
                    writeEvidence('cutover_after_key_result.json', { ts: nowIso(), uuid, exit_code: code ?? null });
                  } catch {}

                  try {
                    const r = await postJson(`${controlPlane}/api/delivery/mark_delivered`, { uuid }, 12000);
                    writeEvidence('mark_delivered_result.json', { ts: nowIso(), uuid, ok: Boolean(r?.ok), status: r?.status ?? null, json: r?.json ?? null });
                  } catch (e) {
                    writeEvidence('mark_delivered_result.json', { ts: nowIso(), uuid, ok: false, error: String(e?.message || e) });
                  }
                });
              } catch (e) {
                writeEvidence('cutover_after_key_result.json', { ts: nowIso(), uuid, ok: false, error: String(e?.message || e) });
              }
            }, 0);
