  ...
  try {
    ...
    setTimeout(() => {
      try {
        ...
        // Scheme-2 idempotency: do NOT restart / refresh QR too aggressively.
        // Repeated start calls will invalidate the previously rendered QR, leading to "scan expired".
        // Only force-start when explicitly requested.
        const now = Date.now();
        const lastDispatch = Number(s._lastStartDispatchedAt || 0) || 0;
        const sinceDispatchMs = lastDispatch ? (now - lastDispatch) : 1e12;

        // If we're already connected, no need to start.
        if (!force && s.status) {
          console.log(`[bothook-provision] wa.start ignored uuid=${uuid} reason=already_connected`);
          return;
        }

        // If a login is already in flight, don't start another.
        if (!force && s._startInFlight) {
          console.log(`[bothook-provision] wa.start ignored uuid=${uuid} reason=in_flight`);
          return;
        }

        // If we already have a QR recently, avoid refreshing it (keep it stable for scanning).
        const hasQr = Boolean(s.lastQrDataUrl);
        const qrFreshMs = (() => {
          try { return s.lastQrAt ? (now - Date.parse(String(s.lastQrAt))) : 1e12; } catch { return 1e12; }
        })();
        if (!force && hasQr && qrFreshMs < 60_000) {
          console.log(`[bothook-provision] wa.start ignored uuid=${uuid} reason=qr_recent ageMs=${Math.floor(qrFreshMs)}`);
          return;
        }

        // Global per-uuid cooldown to prevent start storms.
        if (!force && sinceDispatchMs < 15_000) {
          console.log(`[bothook-provision] wa.start ignored uuid=${uuid} reason=cooldown sinceMs=${Math.floor(sinceDispatchMs)}`);
          return;
        }

        s._lastStartDispatchedAt = now;
