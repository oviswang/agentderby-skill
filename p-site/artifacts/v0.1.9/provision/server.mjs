    ...
    sock.ev.on('connection.update', async (u) => {
      ...
      try {
        ...
        if (u.connection === 'open') {
          sess.connected = true;
          sess.restartDelayMs = 800;
          if (sess._restartTimer) { try { clearTimeout(sess._restartTimer); } catch {} sess._restartTimer = null; }
          try {
        ...
        if (u.connection === 'close') {
          ...
          // Auto-restart to obtain a fresh QR, but avoid restart storms.
          // Only restart on known QR/session expiry or transient network issues.
          ...
          const shouldRestart = (
            !sess.connected && (
              ...
              sc === 408 || sc === 502 || sc === 503
            )
          );

          if (shouldRestart) {
            // exponential backoff (800ms -> 30s), reset on successful open
            sess.restartDelayMs = Math.min(30000, Math.max(800, Math.floor((sess.restartDelayMs || 800) * 1.6)));
            if (!sess._restartTimer) {
              sess._restartTimer = setTimeout(() => {
                sess._restartTimer = null;
