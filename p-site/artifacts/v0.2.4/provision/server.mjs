...
function shAsync(cmd, { timeoutMs = 8000 } = {}) {
  return new Promise((resolve) => {
    ...
    child.stdout.on('data', (d) => { stdout += d.toString('utf8'); if (stdout.length > 2_000_000) stdout = stdout.slice(-1_000_000); });
    child.stderr.on('data', (d) => { stderr += d.toString('utf8'); if (stderr.length > 2_000_000) stderr = stderr.slice(-1_000_000); });
    child.on('close', (code) => {
      clearTimeout(timer);
...
function shFireAndForget(cmd, { timeoutMs = 4000 } = {}) {
  // Never block the HTTP handler.
  shAsync(cmd, { timeoutMs }).catch(() => {});
}

function stopGateway(){
  // Avoid login competition. Must be non-blocking.
  shFireAndForget('timeout 2 systemctl stop openclaw-gateway.service || true', { timeoutMs: 4000 });
  shFireAndForget('timeout 2 systemctl --user stop openclaw-gateway.service || true', { timeoutMs: 4000 });
}

function startGateway(){
  shFireAndForget('timeout 2 systemctl start openclaw-gateway.service || true', { timeoutMs: 4000 });
  shFireAndForget('timeout 2 systemctl --user start openclaw-gateway.service || true', { timeoutMs: 4000 });
}
...
async function pollStatus(uuid){
  ...
  try {
    const r = await shAsync('openclaw channels status', { timeoutMs: 5000 });
