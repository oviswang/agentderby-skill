...
ARTIFACT_BASE_URL="${ARTIFACT_BASE_URL:-https://p.bothook.me/artifacts/v0.1.1}"
...
main(){
  ...
  # Enable gateway to persist across reboot/idle. It will wait for config file before starting.
  systemctl enable openclaw-gateway.service || true

  log "Bootstrap done. Next: when OpenClaw config exists, gateway will start via systemd."
}
