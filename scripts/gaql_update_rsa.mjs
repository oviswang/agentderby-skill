#!/usr/bin/env node
/**
 * Update (replace) Responsive Search Ad assets for an existing ad_group_ad.
 *
 * Usage:
 *   node scripts/gaql_update_rsa.mjs --adgroupAd 194116886055~798865165652 --finalUrl https://bothook.me/ \
 *     --assets growth/rsa_assets_en_v0.1.json
 */
...
function parseArgs(argv) {
  const out = { adGroupAd: null, finalUrl: 'https://bothook.me/', assetsPath: null };
  for (let i = 2; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--adgroupAd') out.adGroupAd = String(argv[++i]);
...
async function main() {
  ...
  if (!args.adGroupAd) throw new Error('missing --adgroupAd <adGroupId~adId>');
  ...
  const adGroupAdRes = `customers/${customerId}/adGroupAds/${args.adGroupAd}`;

  const operation = {
    update: {
      resourceName: adGroupAdRes,
      ad: {
        // Keep the existing ad resource; we replace RSA assets.
        responsiveSearchAd: {
    ...
    updateMask: 'ad.responsive_search_ad.headlines,ad.responsive_search_ad.descriptions,ad.responsive_search_ad.path1,ad.responsive_search_ad.path2,ad.final_urls'
  };
  ...
  const res = await httpJson('POST', url, { headers, body: { operations: [operation], partialFailure: false }, timeoutMs: 30000 });
  console.log(JSON.stringify({ ok: true, updated: args.adGroupAd, result: res }, null, 2));
}
