#!/usr/bin/env node
/**
 * Add NEGATIVE keyword criteria to an ad group.
 *
 * Usage:
 *   node scripts/gaql_adgroup_add_negative_keywords.mjs --adgroup 194116886055 --match PHRASE --kw "apk" --kw "crack"
 */
...
async function main() {
  ...
  const kws = Array.from(new Set(args.kws.map(normKw).filter(Boolean)));

  const operations = kws.map(text => ({
    create: {
      adGroup: adGroupRes,
      status: 'ENABLED',
      negative: true,
  ...
  console.log(JSON.stringify({ ok: true, adGroupId: args.adGroupId, negative_match: args.match, added_attempted: kws.length, results }, null, 2));
}
